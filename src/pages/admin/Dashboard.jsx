      ...
      <div className="grid lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/admin/complaints')} className="text-emerald-700 text-xs hover:underline flex items-center gap-1">View All <ChevronRight size={14} /></button>
          </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/admin/requests')} className="text-emerald-700 text-xs hover:underline flex items-center gap-1">View All <ChevronRight size={14} /></button>
          </div>
          <div className="space-y-3">
            {recentRequests.length === 0 ? <p className="text-gray-400 text-sm text-center py-4">No pending requests</p> : recentRequests.map(r => (
              <div key={r.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => navigate('/admin/requests')} className="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-lg hover:bg-emerald-100">Review</button>
                </div>
              </div>
            ))}
          </div>
