...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen animated-gradient flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="hidden sm:block absolute bottom-1/4 right-1/4 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl animate-float pointer-events-none" style={{ animationDelay: '1.5s' }} />

      {/* Card wrapper — full width on mobile, 50% on desktop */}
      <div className="w-full sm:w-[90%] md:w-[60%] lg:w-[45%] xl:w-[38%] max-w-lg relative z-10">
        ...
        <div
          ...
          style={{ boxShadow: '0 25px 60px rgba(0,0,0,0.3), 0 0 40px rgba(59,130,246,0.15)' }}
        >
          {/* Header */}
          <div className="animated-gradient px-6 py-8 sm:p-8 text-center relative overflow-hidden">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">CampusPlus</h1>
            <p className="text-blue-200 text-xs sm:text-sm mt-1">One Campus. One Platform. Zero Confusion.</p>
          </div>
          ...
          <div className="px-5 py-6 sm:p-8">
            ...
            <div className="flex bg-gray-100 rounded-2xl p-1 mb-5 sm:mb-6">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 ${role === 'student' ? 'bg-white shadow-md text-blue-600 scale-[1.02]' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <GraduationCap size={15} /> Student
              </button>
              <button
                onClick={() => { setRole('admin'); setError('') }}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 ${role === 'admin' ? 'bg-white shadow-md text-indigo-600 scale-[1.02]' : 'text-gray-500 hover:text-gray-700'}`}
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs sm:text-sm text-red-600">
                  {error}
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-primary-glow w-full justify-center py-3 text-sm sm:text-base"
              >
                {loading
            ...
            <p className="text-center text-xs text-gray-400 mt-4 sm:mt-5">
              ...
              <a href="mailto:admin@campusplus.dev" className="text-blue-600 font-medium hover:underline">
